...
window.portfolioProjectDetails["scf-vietnam"] = {
  ...
  "popupSections": [
    ...
    },
    {
      ...
      "body": [
        "**What it is.** Before the bank can run KYC, the system needs to know what kind of dealer it's dealing with.",
        "**The challenge.** Vietnam has no single business ID. A company uses a tax code (MST). A household trader uses a personal ID (CCCD). These two ID types map to three dealer forms. A Business Dealer and an SME both use MST, but their KYC is structured differently. A Personal Dealer uses CCCD. Each form triggers a different bank KYC check. So the type has to be known first. But asking a dealer to classify themselves adds a step many can't answer. That's where people drop off.",
        "**The decision.** So I didn't open one form for everyone. Instead, the Anchor submits a recommendation file first. The system reads that file, identifies the dealer's program, and picks one of three form variants I built, one per dealer type: Business Dealer, SME, and Personal Dealer. Only then does the invite link go out. The dealer opens the link and lands on a form already shaped for their type. They just confirm their identity. If an ID doesn't match the file, the error points at the file, not the dealer. The reason this works: the system already holds the answer, so the dealer never sees the classification logic.",
        "**Trade-off.** This depends on the Anchor's file being accurate. If the file is wrong, the dealer hits an error they can't fix. We accepted that. It's an ops problem upstream of the dealer, not a product one.",
        "**What I did.** I worked out the form-from-file approach with the team. Then I designed the three form variants, the error messages, and the in-context consent placement. The India product used one ID, so it never hit this. The two-ID problem and the file-driven form were built for Vietnam."
      ],
      "carousel": {
    ...
    },
    {
      ...
      "body": [
        "**What it is.** This is distributor finance. The Anchor sells to the dealer and issues the invoice, and that invoice becomes the proof of a real trade. So the product replaces the business plan with this trading history. (The model came from the India product.) After every order, the Anchor uploads an invoice with the dealer, the amount, and the payment date. Each upload adds to a record, and over time that record becomes what the bank evaluates for credit. My job was to make it work across every role that touches it.",
        "**The challenge.** The invoice is now the proof, so a wrong invoice means a wrong credit decision. The Anchor creates the invoices, the dealer's limit depends on them, and the bank reads them to approve credit. If someone uploads the wrong amount or the wrong dealer, the record breaks. The dealer then sees a smaller limit with no way to know why. Trading history only works if you can correct it without erasing the history.",
        "**The decision.** Build invoice handling as a role-based flow, not a one-way upload. The Anchor and Admin create, disable, and re-upload invoices. The dealer views them and selects which to finance, but never creates one. When an invoice is wrong, it gets disabled, not deleted, so the history stays auditable. The dealer then sees which invoices are inactive and why, instead of an unexplained gap in their limit.",
        "**Trade-off.** This only works for dealers inside an active anchor network. A standalone borrower has no invoices to build a record. We accepted that. It's who the product is for.",
        "**What I did.** I mapped how one wrong invoice hits each role, then designed the invoice functions role by role. The core was the inactive-invoice mechanism: a wrong invoice gets disabled and stays visible, not deleted. This was my Vietnam-specific contribution."
      ],
      "carousel": {
    ...
    },
    {
      ...
      "body": [
        "**What it is.** To finish applying, the dealer has to complete a process that runs inside the bank's own system, not SCF's.",
        "**The challenge.** To finish the credit application, the dealer crosses from SCF into the bank's own system on one screen. Two constraints can't be changed. The dealer must log in with the bank directly, because browser policy, security, and banking law all require it. And the bank's embedded UI keeps its own look, which SCF can't restyle. So the real risk is the handoff. The dealer loses their place, can't tell which system they're in, and stalls. For a first-time finance-app user, that's where they drop off.",
        "**The decision.** Accept the two constraints. Own everything else around the embed. Keep the bank's screen between SCF's header and bottom navigation, so the dealer can see they never left SCF. Prefill the bank's form from SCF data, so the dealer re-enters nothing behind the required login. When the dealer returns from the bank, drop them back where they left off in SCF, not at the start. Build a progress tracker, owned by SCF, that turns the bank's internal process into a few readable stages. The dealer can't lose their place, even with the seam still there.",
        "**Trade-off.** Two design languages on one screen: SCF's and the bank's. We accepted it on purpose. A mismatched palette is recoverable. A dealer lost at the door is not.",
        "**What I did.** The embed decision was the client's. I owned making it feel like one product. I designed the SCF flow from scratch, since there was none to embed into. I defined the bank's process as a single stage inside SCF, and set the entry point into it. I designed a place in SCF to surface the bank's messages, so the dealer reads them inside SCF instead of logging into the bank each time."
      ],
      "carousel": {
        "title": "Key decision 03 showcase",
        "slides": [
          {
            "label": "SCF dashboard (home)",
            "description": "The dealer starts on the SCF dashboard. This is home, before they open the application.",
            "imageSrc": "./assets/scf-img-decision03-dashboard.png"
          },
          {
            "label": "Application detail, SCF progress tracker",
            "description": "The application detail, still inside SCF. The SCF-owned progress tracker turns the bank's internal process into a few readable stages, so the dealer knows where they are and what's next.",
            "imageSrc": "./assets/scf-img-decision03-progress-tracker.png"
          },
          {
            "label": "The embed, bank screen inside SCF",
            "description": "The deepest step. The bank's screen sits between SCF's header and bottom navigation, so the dealer can see they never left SCF.",
    ...
    },
    {
      ...
      "body": [
        "**What it is.** A dealer submits a request to draw money, then waits. As the request moves through the bank, its status changes at each step. The dealer has one question: do I need to do anything right now?",
        "**The challenge.** The bank's system tracks eleven different statuses for our partner alone. These are built for banking operations, not for a dealer. Looking at the screen, a dealer can't tell what is happening, whether they need to do something, or whether the bank is still working on it.",
        "The bigger risk is the future. Each bank has its own set of statuses. Show them raw, and every new bank adds another pile. The SCF experience breaks apart from one bank to the next.",
        "**The decision.** SCF becomes the single owner of every status the dealer sees. Every status, from any bank, is translated into one of five plain-Vietnamese badges before it reaches a screen. Badges group by what the dealer should do, not by what the bank is doing. If the bank rejects the request, the dealer still sees what to fix.",
        "The request detail page has three layers, each more specific than the last. The SCF badge says the outcome in one word. The bank's message says why. The invoice table shows the exact invoice to fix. No layer repeats the one above it.",
        "**Trade-off.** Several of the bank's statuses collapse into a single SCF status. So the dealer can't tell exactly which internal step the request is on. We accepted that. Those steps are the bank's internal work. The dealer can't act on them anyway, so showing them would only cause worry.",
        "**Payoff.** When the second bank partner joined with a completely different set of statuses, adding it meant mapping those statuses to the same five badges. A setup task of days, not a redesign of months.",
        "**What I did.** I owned this end to end. I spotted the mismatch and framed the translation as SCF's job, not the bank's. Then I built it.",
        "• I mapped the bank's eleven statuses into five badges, and wrote each badge label in plain Vietnamese. I set the grouping rule: statuses group by what the dealer should do next, not by what the bank is doing internally.",
        "• I designed the three-layer request detail page, from the one-word badge, to the bank's reason, down to the exact invoice to fix.",
        "• I owned the edge cases. I started from the bank's statuses and drew the happy-path flow. Then I mapped every branch where it could break. That gave twelve cases, all derived from the disbursement state machine before a single screen was built. I designed the screen for each one, so a dealer who hits an edge case still sees what to fix."
      ],
      "carousel": {
